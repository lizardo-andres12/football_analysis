from sklearn.cluster import KMeans


class TeamAssigner:
    def __init__(self):
        self.team_colors = {}
        self.player_team_dict = {}
    
    def assign_team_color(self, frame, tracks):
        player_colors = []
        for _, player in tracks.items():
            bbox = player['bbox']
            player_color = self.get_player_color(frame, bbox)
            player_colors.append(player_color)

        kmeans = KMeans(n_clusters=2, init='k-means++', n_init=1).fit(player_colors)

        self.kmeans = kmeans

        self.team_colors[1] = kmeans.cluster_centers_[0]
        self.team_colors[2] = kmeans.cluster_centers_[1]

    def get_player_team(self, frame, player_bbox, player_id):
        if player_id in self.player_team_dict:
            return self.player_team_dict[player_id]

        player_color = self.get_player_color(frame, player_bbox)

        team_id = self.kmeans.predict(player_color.reshape(1, -1))[0]
        team_id += 1

        self.player_team_dict[player_id] = team_id

        return team_id
            
    def get_player_color(self, frame, bbox):
        cropped_image = frame[int(bbox[1]):int(bbox[3]), int(bbox[0]):int(bbox[2])]
        top_half = cropped_image[0:int(cropped_image.shape[0]/2), :]

        # get clusters
        kmeans = self.get_clustering_model(top_half)

        # get labels for each pixel
        labels = kmeans.labels_
        
        # reshape to original image
        clustered_img = labels.reshape(top_half.shape[0], top_half.shape[1])

        # get player cluster
        corner_clusters = [clustered_img[0, 0], clustered_img[0, -1], clustered_img[-1, 0], clustered_img[-1, -1]]
        non_player_class = max(set(corner_clusters), key=corner_clusters.count)
        player_class = 1 - non_player_class

        player_color = kmeans.cluster_centers_[player_class]

        return player_color


    def get_clustering_model(self, image):
        flattened_image = image.reshape(-1, 3)
        
        # lower iterations by using kmeans++
        kmeans = KMeans(n_clusters=2, init='k-means++', n_init=1).fit(flattened_image)

        return kmeans